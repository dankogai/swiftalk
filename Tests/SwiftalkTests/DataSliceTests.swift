...
struct DataSliceTests {
    ...
    func read() throws {
        #expect(try eval("\"hello\".Data(.utf8)[1..<3]") == .data([101, 108]))
        #expect(try eval("\"hello\".Data(.utf8)[3...]") == .data([108, 111]))
        #expect(try eval("\"hello\".Data(.utf8)[1..<3].String(.utf8)") == .string("el"))
        #expect(try eval("\"hello\".Data(.utf8)[5...]") == .data([]))
        #expect(try eval("\"hello\".Data(.utf8)[1..<3].Type == Data") == .bool(true))
        #expect(try eval("let d = Data([1, 2, 3])\nlet e = d[1...]\nd") == .data([1, 2, 3]))
        #expect(throws: SwiftalkError.self) { try eval("\"hello\".Data(.utf8)[2...9]") }
        #expect(throws: SwiftalkError.self) { try eval("\"hello\".Data(.utf8)[-1...1]") }
        #expect(throws: SwiftalkError.self) { try eval("\"hello\".Data(.utf8)[\"x\"]") }
    }

    @Test("d[0..<1] = Data(...) is replaceSubrange; d[i] = byte writes one byte in 0...255")
    func write() throws {
        #expect(try eval("var d = \"hello\".Data(.utf8)\nd[0] = 72\nd.String(.utf8)") == .string("Hello"))
