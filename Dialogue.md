...
* **2026-09-16, round 162 — `r.then { v in }`** ("Add `res.then { v in
  ... }` too"). Round 161's "no `then`" lasted a day; the chain is
  the point — `eval(s).then { ... }.then { ... }.catch { ... }` reads
  as a program, and `?` only helps inside a function. A success maps
