...
/// A type is a value (§10), so `let I = Int` aliases it — and an
/// annotation naming such a binding means the type it holds (round
/// 111, which retracted round 110's `typealias`). Resolved once, where
/// the annotation is used; locks are stored resolved.
func resolveTypeNames(_ annotation: TypeAnnotation, in env: Environment) -> TypeAnnotation {
    ...
    if !knownTypeNames.contains(name), case .function(let f)? = try? env.lookup(name) {
        ...
        case .structType(let st): name = st.name
        case .actorType(let at):  name = at.name
        default: break
        }
    }
    return TypeAnnotation(name: name, optional: annotation.optional,
                          parameters: annotation.parameters.map { resolveTypeNames($0, in: env) })
}
...
private func executeSlow(_ statement: Stmt, in env: Environment, relaxed: Bool) throws -> Value {
    ...
    case .declaration(let mutable, let name, let rawAnnotation, let initializer):
        ...
        let annotation = rawAnnotation.map { resolveTypeNames($0, in: env) }   // let I = Int (round 111)
    ...
    case .enumDecl(let name, let caseOrder, let cases, let methodExprs):
        ...
        for (caseName, payloads) in cases {
            cases[caseName] = payloads.map { payload in
                guard let t = payload.typeName else { return payload }
                return (label: payload.label,
                        typeName: resolveTypeNames(TypeAnnotation(name: t, optional: false), in: env).name)
            }
        }
    ...
    case .structDecl(let name, let propertyOrder, let properties, let methodExprs,
        ...
        for (key, prop) in properties {
            ...
            properties[key] = StructType.Property(mutable: prop.mutable, annotation: resolveTypeNames(a, in: env),
