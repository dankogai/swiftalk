...
| `Sequence.zip(s, t)` | a lazy Sequence of `(x, y)` pairs until the shorter side ends (round 174; a static of Sequence since round 185, `zip` at the top level until then) — see [toplevel.md](toplevel.md); `Array(zip(1..., "ab"))` is `[(1, "a"), (2, "b")]` |
