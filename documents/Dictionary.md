...
| `d.keys` | a **Set** of the keys (round 132, revising 127's Array): unordered and unique, so `d0 == d1` implies `d0.keys == d1.keys`; `d.keys.sorted()` for a list. A property, not a call: `d.keys()` is an error that says so; a `Set<K>` of a `[K: V]`'s `K`, even when empty (round 180) |
| `d.values` | an **Array** of the values, in the Dictionary's own order — the order `for k, v in d` walks (round 127); a `[V]`, even when empty (round 180) |
| `d == e` | equality |
