...
| `let String = { $.contains(.pretty) ? "…" : .description }`, `let Double = { … }` | **conversion members** (rounds 47, 151): `p.Double()` and `Double(p)` both run the member, format arguments as `$`; a `String` member owns `p.String(...)` and is asked for `p`'s text at any depth of a `.pretty` layout (`[p].String(.pretty)`); the plain `.String()` of a container stays the source form; `.description` inside the member is the builtin text (`String(self)` would recurse) |
