...
| `p.description`, `p.debugDescription`, `p.String(.canonical)` | the **builtin** memberwise text, `"Point(x: 3, y: 4)"`, never the member's — so a `String` member may return `.description` for the forms it leaves alone (`String(self)` would recurse); `.String(.canonical)` (round 153) is the same text as a format word, at any depth (`[p].String(.canonical)`) and with `.pretty`; the member never receives `.canonical`. `.String(.sion)`/`.json` on a container are data formats and stay builtin too |
