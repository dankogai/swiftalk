...
| `v.String(.pretty)`, `v.String(.json, .pretty)` | the same SION / JSON text laid out one element per line, two spaces a level — Arrays, Dictionaries, tuples, structs, and enum payloads open up (rounds 117–118), everything else stays on its line; `.pretty` alone means `.sion` |
