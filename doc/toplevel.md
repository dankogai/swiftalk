...
type, `map` infers one from its results, `Array(x)`, `Set(x)`, and `Dictionary(pairs)` keep or infer, `zip` and `enumerated()` are `[Tuple]` (rounds 168–175); an optional is its own type, `Int? != Int`,
