...
| `a.enumerated()` | an Array of `(key:, value:)` tuples — the index as `key` (round 73; labels round 128, a divergence from Swift's `offset:`/`element:` so a Dictionary's pairs and an Array's look alike) |
