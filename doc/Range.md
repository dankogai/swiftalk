...
| `a[1..<3]`, `a[1...]` | a Range as an Array subscript (round 90) — see [Array.md](Array.md) |
| `a...` | **the unbounded range** (round 88): infinite, lazy — `for i in 0... { ... break }`, `(0...).prefix(5)`, `(1...)[3]`, `case 40...:`; `map`/`filter`/`enumerated`/`prefix { }`/`dropFirst { }` defer on it (a Sequence comes back); `count`, `reduce`, `sorted`, `reversed`, `joined`, `Array()` refuse it; the element after `Int.max` is an overflow error. `a..<` has no unbounded form |
| `r.prefix { }`, `r.dropFirst { }` | the leading elements while the predicate holds / everything from its first miss — an Array on a bounded range, lazy on `a...` (round 88) |
