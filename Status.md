...
swiftalk> Double.random(max: 1)
type error: Double.random(), .random(to:), or .random(from:to:)
swiftalk> Int.random(in: Int.min...Int.max).Type
Int
swiftalk> Int.random(from: 0, to: 6)
type error: Int.random(in:) takes one Range: Int.random(in: 1...6)
```
...
Data
swiftalk> 42.D()
type error: cannot convert Int to Data
swiftalk> typealias N = Int
syntax error: expected a newline or ';' between statements
```
