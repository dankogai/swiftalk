# swiftalk by example — the `examples/` directory

Runnable examples (round 66). Run any of them with:

```sh
swift run swiftalk examples/lambda.swt
```
...
Every example is verified by the test suite (`ExamplesTests`) — the quine
...
* **[quine-print.swt](quine-print.swt)** — the *printing* quine:
  `swift run swiftalk examples/quine-print.swt` outputs its own source,
